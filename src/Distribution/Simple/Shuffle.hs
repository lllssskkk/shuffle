...
import Distribution.Parsec (parsecFilePath, parsecOptCommaList, unPP)
...
import System.IO (IOMode (..), hClose, hFileSize, hGetLine, hIsEOF, hPutStrLn, openFile, withFile)
import qualified Text.Parsec as Parsec
import UHC.Shuffle (FPathWithAlias, Opts, getDeps, parseOpts, shuffleCompile)
import UHC.Util.FPath (FPath, fpathFromStr, fpathGetModificationTime)
...
prepCHS ignore outDir bi verbosity = do
  fs <- forM (hsSourceDirs bi) $ \dir -> do
    contents <- getDirectoryContentsRecursive $ getSymbolicPath dir
    let chs = filter ((== ".chs") . takeExtension) contents
    let chs' = filter (not . (`elem` ignore)) chs
    fs <- forM chs' $ \file -> do
      let outFile = outDir </> replaceExtension file "hs"
      empt <- preprocess bi "hs" (normalise $ (getSymbolicPath dir) </> file) outFile verbosity
...
generateAG outDir bi verbosity files = do
  ...
  deps <- forM files $ \inFile -> do
    mbPath <- findFileWithExtension' [takeExtension inFile] (fmap getSymbolicPath $ hsSourceDirs bi) (dropExtension inFile)
    case mbPath of
      Nothing -> dieNoVerbosity $ "can't find source for " ++ inFile ++ " in " ++ intercalate ", " (fmap getSymbolicPath $ hsSourceDirs bi)
      Just (dir, file) -> do
...
shuffleBuildHook origBuildHook pd lbi hook bf = do
  ...
  let addOpts :: FilePath -> BuildInfo -> IO ([ModuleName], BuildInfo)
      addOpts outDir bi = do
        ...
        let newBi =
              bi
                { customFieldsBI = dataOpts ++ semOpts ++ semDataOpts ++ customFieldsBI bi,
                  hsSourceDirs = unsafeMakeSymbolicPath outDir : hsSourceDirs bi
                }
        return $ (mods, newBi)
...
shouldRebuild optstr inFile outFile = do
  ...
  if exists
    then do
      timeIn <- fpathGetModificationTime (fpathFromStr inFile)
...
chsFiles ignore bi = do
  fs <- forM (hsSourceDirs bi) $ \dir -> do
    contents <- getDirectoryContentsRecursive . getSymbolicPath $ dir
    return $
      map (\file -> normalise $ (getSymbolicPath dir) </> file) $
